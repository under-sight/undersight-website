...
`CMS/Pages` "Blog - *" entities have been replaced with proper
`CMS/Blog` entries.

**Status:** PENDING. Do not execute these edits until:

1. User has added the 6 required fields to `CMS/Blog` in Fibery UI
   (see migration script for the list).
2. Migration script has been run successfully (no errors, all 7 entities
   merged).
3. User has manually verified the migrated `CMS/Blog` entries in Fibery.
...
4. **Manual validation** in Fibery UI: open each of the 7 `CMS/Blog`
...
9. **User reviews production**, then manually deletes the 7 `Blog - *`
   source entries in `CMS/Pages` plus B8 ("Test Blog") and B9 (null
   stub) in `CMS/Blog` via the Fibery UI.
...
**Add a 2nd query** for `CMS/Blog`. Project as `_blogs` array in the
content map (similar pattern to existing `_whitepapers`).

- Query fields: `CMS/name`, `CMS/Slug`, `CMS/Type`,
  `CMS/Description` (doc), `CMS/Assets`, `CMS/Post Date`,
  `CMS/Subtitle`, `CMS/Author`, `CMS/Excerpt`, `CMS/PDF`.
- Sort: `CMS/Post Date` desc, tie-break `fibery/creation-date` desc.
- The existing `CMS/Pages` query continues to power Home, Solutions,
  SEO, Site Config, etc. — just stop using `Blog - *` entries from it.

The whitepaper-lookup query in `_capture_lead()` already targets
`CMS/Blog` by name, so it requires no change.

### 2. `build.py` — static build

Mirror the same delta as `undersight-serve.py`:

- Add a 2nd query against `CMS/Blog`, project into the content map
  under a `_blogs` key.
...
(data._blogs || []).map(blog => ({
  ...
  body:      blog.body,    // markdown from CMS/Description
  files:     blog.files,   // CMS/Assets
  pdf:       blog.name,    // for whitepaper modal lookup
}))
```

Each blog object now arrives pre-structured from Fibery — no more
front-matter parsing. `parseMeta(md)` stays in use for other `CMS/Pages`
...
Fibery `CMS/Blog` entity name. Already true with the new data shape.

### 4. `functions/api/whitepaper-lead.js` — Cloudflare Pages function

**No change needed.** This function already queries `CMS/Blog` by name
...
```bash
# Fail build if any "Blog - *" entity remains in CMS/Pages.
python3 -c "
import json, subprocess
out = subprocess.run(['/Users/kyle/bin/fibery', 'undersight', 'query',
    '--query-json', json.dumps({
        'q/from': 'CMS/Pages',
        'q/where': ['q/contains', ['CMS/name'], 'Blog -'],
        'q/select': {'name': ['CMS/name']},
        'q/limit': 50,
    })], capture_output=True, text=True, check=True)
data = json.loads(out.stdout)
if data:
    print('GUARD FAIL: Stale Blog - * entities in CMS/Pages:')
    for d in data: print(f'  - {d[\"name\"]}')
    raise SystemExit(1)
print('Schema guard OK: no stale Blog - * entities in CMS/Pages')
"
```
...
- **Existing leads** already in `CMS/Blog Leads` (linked via
  `CMS/Blog Post` relation) keep their links intact — we're not
...
- The 7 source `Blog - *` entities in `CMS/Pages` are **left intact**
  by the migration script. Code changes can be reverted via `git revert`
  to fall back to the old path.
- The migration is purely additive on the `CMS/Blog` side — running it
  again is idempotent (no destructive writes).
- Manual cleanup of `CMS/Pages` entries is the **last** step and
