...
export default {
  async fetch(request, env) {
    ...
    try {
      // 0. Suppression check — unsubscribed addresses get a generic OK with
      // no lead created (and therefore no dispatch email). Fibery is the
      // source of truth (see functions/unsubscribe.js).
      ...
      // 2. Create the lead, linking to the blog post. The unsubscribe token
      // is generated here so the dispatch email can interpolate a one-click
